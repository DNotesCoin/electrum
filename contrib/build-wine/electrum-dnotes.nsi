...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-dnotes-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "..\..\icons\electrum-dnotes.ico"
...
Section
  ...
  File /r "dist\electrum-dnotes\*.*"
  File "..\..\icons\electrum-dnotes.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-dnotes-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-dnotes-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-dnotes-${PRODUCT_VERSION}.exe" 0
  ...
  WriteRegStr HKCU "Software\Classes\dnotes\shell\open\command" "" "$\"$INSTDIR\electrum-dnotes-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrum-dnotes.ico"
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\dnotes"
