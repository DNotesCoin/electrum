...
class RequestsDialog(Factory.Popup):
    ...
    def update(self):
        ...
        _list = self.app.wallet.get_sorted_requests(self.app.electrum-dnotes_config)
        for pr in _list:
            ci = self.get_card(pr)
    ...
    def do_delete(self, req):
        ...
        def cb(result):
            if result:
                self.app.wallet.remove_payment_request(req.address, self.app.electrum-dnotes_config)
                self.hide_menu()
                self.update()
