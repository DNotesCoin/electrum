...
class SendScreen(CScreen):
    ...
    def do_send(self):
        if self.screen.is_pr:
            ...
            if not address:
                self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a DNotes address or a payment request'))
                return
            if not bitcoin.is_address(address):
                self.app.show_error(_('Invalid DNotes Address') + ':\n' + address)
                return
            try:
        ...
        if self.app.electrum-dnotes_config.get('use_rbf'):
    ...
    def _do_send(self, amount, message, outputs, rbf):
        ...
        config = self.app.electrum-dnotes_config
...
class ReceiveScreen(CScreen):
    ...
    def on_address(self, addr):
        req = self.app.wallet.get_payment_request(addr, self.app.electrum-dnotes_config)
        self.screen.status = ''
        if req:
    ...
    def do_share(self):
        uri = self.get_URI()
        self.app.do_share(uri, _("Share DNotes Request"))
    ...
    def save_request(self):
        ...
        try:
            self.app.wallet.add_payment_request(req, self.app.electrum-dnotes_config)
            added_request = True
        except Exception as e:
